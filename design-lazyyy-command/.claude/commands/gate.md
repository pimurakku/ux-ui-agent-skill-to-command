...
Do not edit `project.yml` by hand. One command writes the result, and it is the
same one the unattended pipeline uses:

```bash
python3 tools/state.py gate G1 pass --evidence <file you read> <file you read>
python3 tools/state.py gate G1 fail --missing "<what was missing>" "<and this>"
```

Why a command rather than an edit. `stage:` and `gates:` have exactly one writer
in this repository, so "advance only on a real pass" is enforced in one function
instead of being a rule every role could break by accident. A pass without
evidence is refused outright — that is the thing `GAT-01` exists to catch, and it
should not be possible to write in the first place. A pass advances `stage:` to
the next one for you; a failure does not, and nothing ever deletes an earlier
entry. The record of what failed and why is what the student comes back to.
