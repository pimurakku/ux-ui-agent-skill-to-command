...
description: Own the design stage of this pipeline and the structure of a design system across many components and products — foundations, component architecture, governance, documentation standards, versioning and breaking changes, multi-brand support, and the DSN design specification document. Use this skill for design system audits, deciding what belongs in the system versus a product, structuring a component inventory, system health and adoption, and whenever the design stage needs its registered deliverable. Do NOT use it for the visual choices on a single screen — that is design-fundamentals; not for token file formats, transformation or distribution — that is design-tokens-system; and not for packaging and publishing a component library — that is component-library-mastery.
...
This skill owns the design stage's deliverable in this pipeline, so it also owns
the questions that stage depends on.

1. Read `project/project.yml` first. If it does not exist, run `project-intake`.
2. Ask only what is still blank. The questions are in `project-intake`'s
   `references/question-bank.md`, round 4: whether a brand or style guide
   already exists, which devices are primary, and whether this extends an
   existing design system or starts from zero. Extending and starting fresh are
   different-sized jobs, and token collisions with an existing system are
   expensive to discover late.
3. Write the answers back to `project.yml` under `design`.
4. Produce `DSN-XXX-001` from `assets/dsn-template.md` and register it. Without
   a registered design document, `quality-assurance` has nothing to compare the
   build against, and the visual dimension of that gate cannot run.
